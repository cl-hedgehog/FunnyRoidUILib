 ...
 * @Description: 进度圆形进度条，线程安全的View，可直接在线程中更新进度
 * @date 2016/7/6 15:16
 */
public class RoundProgressBar extends View {
    ...
    private void drawProgressCircle(Canvas canvas) {
        ...
        int currProgress = 360 * progress / max;
        currProgress = currProgress / DRAW_COUNT * drawCount;
        switch (style) {
            case STROKE: {
            ...
            case FILL: {
                ...
                if (progress != 0) {
                    canvas.drawArc(oval, -90, currProgress, true, paint);  //根据进度画圆弧
                }
                break;
            }
        }
    ...
    private void drawProgressText(Canvas canvas) {
        ...
        int percent = progress / 10;  //中间的进度level, 0,1,2,3
        float textWidth = paint.measureText(percent + "");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间

        if (textIsDisplayable && style == STROKE) {
            canvas.drawText(String.valueOf(percent), center - textWidth / 2, center + textSize / 2 - roundProgressWidth + roundWidth, paint);
            //画出进度百分比
        }
    }
